...
CMaterial CEffect2::sMaterial; //マテリアル.テクスチャ

CEffect2::CEffect2(const CVector& pos, float w, float h, char* texture, int row, int col, int fps)
...
void CEffect2::Update() {
	//プレイヤーの位置に追従させる
	CVector pos = CXPlayer::GetInstance()->mPosition;
	pos.mY += 2.0f;
	mPosition = pos;
